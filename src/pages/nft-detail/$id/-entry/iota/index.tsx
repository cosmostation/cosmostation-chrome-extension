...
export default function Iota({ id }: IotaProps) {
  ...
  const selectedNFT = currentAccountAddNFTsWithMeta.iota.find((nft) => nft.id === id);
  ...
  const chain = chainList.iotaChains?.find((chain) => chain.id === selectedNFT?.chainId && chain.chainType === selectedNFT.chainType);

  const errorMessage = (() => {
    return t('pages.nft-detail.$id.entry.iota.index.lockedFeature');
  })();
    ...
    <>
      <BaseBody>
        <ContentsContainer>
          ...
          >
            <DetailRowContainer>
              <Base1000Text variant="b3_R">{t('pages.nft-detail.$id.entry.iota.index.network')}</Base1000Text>
              <ChainContainer>
            ...
            <DetailRowContainer>
              <Base1000Text variant="b3_R">{t('pages.nft-detail.$id.entry.iota.index.owner')}</Base1000Text>

              <Base1300Text variant="b3_M">{shorterOwnerAddress}</Base1300Text>
            </DetailRowContainer>
            <DetailRowContainer>
              <Base1000Text variant="b3_R">{t('pages.nft-detail.$id.entry.iota.index.objectId')}</Base1000Text>
      ...
      >
        <Tooltip title={errorMessage} varient="error" placement="top">
          <div>
            ...
            >
              {t('pages.nft-detail.$id.entry.iota.index.nftSend')}
            </Button>
          </div>
        </Tooltip>
      </StickyFooterInnerBody>

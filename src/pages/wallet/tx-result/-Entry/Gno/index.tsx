...
import { useTxInfo } from '@/hooks/gno/useTxInfo.ts';
...
export default function Gno({ coinId, txHash, address }: GnoProps) {
  ...
  const txInfo = useTxInfo({
    coinId,
    txHash: txHash ? encodeURIComponent(txHash) : undefined,
  });
  ...
  const txConfirmedStatus = useMemo(() => {
    ...
    if (txInfo?.data?.result?.tx_result?.ResponseBase?.Error !== undefined) {
      if (txInfo.data.result.tx_result.ResponseBase.Error !== null) return TX_CONFIRMED_STATUS.FAILED;

      return TX_CONFIRMED_STATUS.CONFIRMED;
    }

    return undefined;
  }, [txInfo?.data?.result?.tx_result?.ResponseBase?.Error, txInfo.error]);

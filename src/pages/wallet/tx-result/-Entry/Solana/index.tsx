...
import { useGetTransaction } from '@/hooks/solana/useGetTransaction.ts';
...
export default function Solana({ coinId, txHash, address }: SolanaProps) {
  const { t } = useTranslation();
  const navigate = useNavigate();

  const { getSolanaAccountAsset } = useGetAccountAsset({ coinId });

  const txInfo = useGetTransaction({
    coinId,
    signature: txHash,
  });
  ...
  const txConfirmedStatus = (() => {
    ...
    if (txInfo.data?.meta) {
      return !txInfo.data.meta.err ? TX_CONFIRMED_STATUS.CONFIRMED : TX_CONFIRMED_STATUS.FAILED;
    }

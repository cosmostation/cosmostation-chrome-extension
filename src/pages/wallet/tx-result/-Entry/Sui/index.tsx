...
import { SUI_TX_CONFIRMED_STATUS } from '@/constants/cosmos/txStatus.ts';
...
export default function Sui({ coinId, txHash, address }: SuiProps) {
  ...
  const txConfirmedStatus = (() => {
    if (txInfo.error?.message === TRASACTION_RECEIPT_ERROR_MESSAGE.PENDING) return SUI_TX_CONFIRMED_STATUS.PENDING;

    if (txInfo.data?.result?.effects?.status.status) {
      if (txInfo.data.result.effects.status.status === TRANSACTION_RESULT.FAILURE) return SUI_TX_CONFIRMED_STATUS.FAILED;

      if (txInfo.data.result.effects.status.status === TRANSACTION_RESULT.SUCCESS) return SUI_TX_CONFIRMED_STATUS.CONFIRMED;
    }

    return undefined;
  })();

  const isTxConfirmed = txConfirmedStatus === SUI_TX_CONFIRMED_STATUS.CONFIRMED;
  const isTxFailed = txConfirmedStatus === SUI_TX_CONFIRMED_STATUS.FAILED || !txHash || txInfo.error;

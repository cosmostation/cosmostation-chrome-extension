...
import { useTxInfo } from '@/hooks/aptos/useTxInfo.ts';
...
export default function Sui({ coinId, txHash, address }: AptosProps) {
  ...
  const selectedAsset = getAptosAccountAsset();
  ...
  const txConfirmedStatus = (() => {
    ...
    if (txInfo.data) {
      return txInfo.data.success ? TX_CONFIRMED_STATUS.CONFIRMED : TX_CONFIRMED_STATUS.FAILED;
    }

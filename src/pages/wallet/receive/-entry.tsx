...
export default function Entry() {
  ...
  const currentSelectedChain = flatChainList.find((chain) => chain.id === currentSelectedChainId);
          ...
          <StickyContentsContainer>
            <FilterContaienr>
              <StyledInput
                ...
                placeholder={t('pages.wallet.receive.entry.search')}
                value={search}
                onChange={(event) => {
            ...
            optionButtonProps={[
              {
                ...
                children: <Typography variant="b2_M">{t('pages.wallet.receive.entry.valueHighOrder')}</Typography>,
              },
              {
                sortKey: DASHBOARD_COIN_SORT_KEY.ALPHABETICAL_ASC,
                children: <Typography variant="b2_M">{t('pages.wallet.receive.entry.alphabeticalAsc')}</Typography>,
              },
            ]}

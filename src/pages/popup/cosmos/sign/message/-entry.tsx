...
export default function Entry({ request, chain }: EntryProps) {
  ...
  const currentCosmosChain = useMemo(() => {
    const inAppSelectedPubkeyStyle = currentPreferAccountType[chain.id].pubkeyStyle;

    const response = produce(chain, (draft) => {
      draft.accountTypes = draft.accountTypes.filter((item) => item.pubkeyStyle === inAppSelectedPubkeyStyle);
    });

    return response;
  }, [chain, currentPreferAccountType]);

  const keyPair = useMemo(() => getKeypair(currentCosmosChain, currentAccount, currentPassword), [currentAccount, currentCosmosChain, currentPassword]);

  const address = useMemo(() => getAddress(currentCosmosChain, keyPair?.publicKey), [currentCosmosChain, keyPair?.publicKey]);
    ...
    try {
      ...
      const signature = await (async () => {
        if (currentAccount.type === 'MNEMONIC' || currentAccount.type === 'PRIVATE_KEY') {
          ...
          return signAmino(tx, privateKeyBuffer, currentCosmosChain);
        }
      ...
      const publicKeyType = currentCosmosChain.accountTypes[0].pubkeyType
        ? getPublicKeyType(currentCosmosChain.accountTypes[0].pubkeyType)

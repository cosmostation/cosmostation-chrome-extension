...
import { createTransport as baseCreateTransport } from '~/Popup/utils/ledger';
...
export function useLedgerCosmos() {
  ...
  return async function CosmosWithTranslation() {
    if (!transport.current) {
      transport.current = await baseCreateTransport('HID');
    }

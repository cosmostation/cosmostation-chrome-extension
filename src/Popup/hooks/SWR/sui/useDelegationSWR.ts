...
export function useDelegationSWR({ address, network }: UseDelegationSWR, config?: SWRConfiguration) {
  ...
  const delegation = useMemo(
    () => ({
      totalStakedAmount: suiStakes.data?.result?.reduce(
      ...
      stakedObjects: suiStakes.data?.result?.map((stake) => ({
        ...stake,
        validator: latestSuiSystemState.data?.result?.activeValidators.find((activeValidator) =>
          isEqualsIgnoringCase(activeValidator.suiAddress, stake.validatorAddress),
        ),
      })),
    }),

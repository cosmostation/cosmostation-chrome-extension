...
import type { AssetV2IbcPayload } from '~/types/cosmos/asset';
...
export function useCoinAssetsSWR(chain?: CosmosChain, config?: SWRConfiguration) {
  ...
  const fetcher = async (fetchUrl: string) => {
    try {
      return await get<AssetV2IbcPayload>(fetchUrl);
    } catch (e: unknown) {
      return null;
    }
  };

  const { data, error, mutate } = useSWR<AssetV2IbcPayload | null, AxiosError>(requestURL, fetcher, {

...
import type { ICNSResponse, OsmosisICNSResponse } from '~/types/cosmos/ICNS';
  ...
  const fetcher = async (props: FetchProps) => {
    try {
      ...
      if (props.chain.id === ARCHWAY.id) {
        ...
        const requestURL = `${ARCHWAY.restURL}/cosmwasm/wasm/v1/contract/archway1275jwjpktae4y4y0cdq274a2m0jnpekhttnfuljm6n59wnpyd62qppqxq0/smart/${nameToBase64}`;

        return await get<ICNSResponse>(requestURL);
      }
      ...
      const requestURL = `${OSMOSIS.restURL}/cosmwasm/wasm/v1/contract/osmo1xk0s8xgktn9x5vwcgtjdxqzadg88fgn33p8u9cnpdxwemvxscvast52cdd/smart/${nameToBase64}`;

      const osmosisICNS = await get<OsmosisICNSResponse>(requestURL);
      return {
        data: {

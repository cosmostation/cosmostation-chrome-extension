...
import { useChainIdToAssetNameMapsSWR } from '~/Popup/hooks/SWR/cosmos/useChainIdToAssetNameMapsSWR';
...
export default function Send({ chain }: CosmosProps) {
  ...
  const availableCoinOrTokenList: CoinOrTokenInfo[] = useMemo(() => {
    const coinOrTokenList = [
      ...coinAll.map((item) => ({
        ...item,
        type: TYPE.COIN,
        name: convertAssetNameToCosmos(item.baseChainName || '', chainIdToAssetNameMaps)?.chainName || getCapitalize(item.baseChainName || ''),
      })),
  ...
  }, [chain.chainName, chain.displayDenom, coinAll, coinGeckoPrice.data, chainIdToAssetNameMaps, cosmosTokensBalance.data, currency, currentCosmosTokens]);

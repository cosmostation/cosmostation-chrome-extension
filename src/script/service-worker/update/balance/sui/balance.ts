...
export async function suiBalances(id: string, { chainId, priority, updateAssets, chunkSize }: BalanceFetchOption = {}) {
  try {
    ...
    for (const chunk of chunks) {
      ...
      await browser.storage.local.set<Pick<ExtensionStorage, `${string}-balance-sui`>>({ [`${id}-balance-sui`]: stored });

      updateAssets?.();
    }
  } catch (error) {
    devLogger.error(`Failed to process suiBalances for account ${id}:`, error);
  }
}

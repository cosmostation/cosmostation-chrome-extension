...
type BitcoinMempoolTxItemProps = {
  tx: AccountTx;
  coinId: string;
};
...
export default function BitcoinMempoolTxItem({ tx, coinId }: BitcoinMempoolTxItemProps) {
  ...
  const isPending = status?.confirmed === false;
  ...
  const detail = (() => {
    if (filteredTxInput?.length || 0 > 0) {
      ...
      return {
        title: t('components.AccountTxHistory.components.Bitcoin.components.BitcoinMempoolTxItem.index.sent'),
        subTitle,
    ...
    return {
      title: t('components.AccountTxHistory.components.Bitcoin.components.BitcoinMempoolTxItem.index.received'),
      subTitle,
    ...
    <TxDetail
      ...
      leftTop={
        <TitleContainer>
          ...
          &nbsp;
          {isPending && (
            <StatusContainer variant="b2_M">
              {t('components.AccountTxHistory.components.Bitcoin.components.BitcoinMempoolTxItem.index.pending')}
            </StatusContainer>
          )}
        </TitleContainer>
      }
      ...
      rightBottom={<Base1000Text variant="h7n_R">{'-'}</Base1000Text>}
    />
  );
}

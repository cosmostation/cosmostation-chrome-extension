...
export default function SolanaFee({ feeCoinId, displayFeeAmount, disableConfirm, isLoading, onClickConfirm }: SolanaFeeProps) {
  const { t } = useTranslation();
  const { data: coinGeckoPrice } = useCoinGeckoPrice();
  const { userCurrencyPreference } = useExtensionStorageStore((state) => state);

  const { getSolanaAccountAsset } = useGetAccountAsset({ coinId: feeCoinId });

  const selectedFeeAsset = getSolanaAccountAsset();

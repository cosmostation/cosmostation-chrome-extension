...
export function useFees({ coinId }: UseFeesProps) {
  const { data: accountAssets } = useAccountAllAssets({ disableDupeEthermint: true, filterByPreferAccountType: true });

  const baseCoinList = useMemo(() => [...(accountAssets?.allGnoAccountAssets || [])], [accountAssets?.allGnoAccountAssets]);
  ...
  return { feeAssets, defaultGasRateKey: assetGasRate.data.defaultGasRateKey };
}

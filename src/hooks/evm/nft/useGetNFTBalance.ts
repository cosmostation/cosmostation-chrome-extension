...
import type { ERC721BalanceOfPayload, ERC1155BalanceOfPayload } from '@/types/evm/contract';
...
export function useGetNFTBalance({ chainId, contractAddress, tokenId, ownerAddress, tokenStandard, config }: UseGetNFTBalanceProps) {
  ...
  const fetcher = async (index = 0) => {
    try {
      ...
      if (tokenStandard === EVM_NFT_STANDARD.ERC721) {
        ...
        const erc721ContractCall = erc721Contract.balanceOf(ownerAddress) as Promise<ERC721BalanceOfPayload>;
  ...
  const { data, isLoading, isFetching, error, refetch } = useFetch({
    queryKey: ['useGetNFTBalance', contractAddress, tokenId, tokenStandard, ownerAddress, chainId],
